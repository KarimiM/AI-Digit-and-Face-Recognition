...
StartingSpace = 4
"""Digits == 1000 test 5000 training Faces == 150 test, 450"""
...
def enhancedFeatureExtractorDigit(datum):
    ...
    features[1] = numWhiteSpaces(datum,basicFeatureExtractorDigit(datum))
    features[2] = numEdge(datum)

    #Code will add a feature for number of contigous white spots with 1, 2, or 3
...
def numWhiteSpaces(datum,features):
    numWhite = 0
    VisitedFlag = util.Counter()
    for x in range(0,28):
        for y in range(0,28):
            VisitedFlag[(x,y)] = 0
    for x in range(0,28):
        for y in range(0,28):
            if features[(x,y)] == 0 and VisitedFlag[(x,y)] == 0:
                L = Queue()
                L.enqueue(x)
                L.enqueue(y)
                VisitedFlag[(x,y)] = 1
                ...
                while True:
                    ...
                    xStar = int(L.dequeue())
                    yStar = int(L.dequeue())
                    if xStar > 0 and VisitedFlag[(xStar - 1,yStar)] == 0 and features[(xStar - 1,yStar)] == 0:
                        #print 1
                        L.enqueue(xStar-1)
                        L.enqueue(yStar)
                        VisitedFlag[(xStar-1,yStar)] = 1
                    if yStar > 0 and VisitedFlag[(xStar,yStar - 1)] == 0 and features[(xStar,yStar - 1)] == 0:
                        #print 2
                        L.enqueue(xStar)
                        L.enqueue(yStar-1)
                        VisitedFlag[(xStar, yStar - 1)] = 1
                    if xStar < 28 and VisitedFlag[(xStar + 1,yStar)] == 0 and features[(xStar + 1,yStar)] == 0:
                        #print 3
                        L.enqueue(xStar + 1)
                        L.enqueue(yStar)
                        VisitedFlag[(xStar + 1, yStar)] = 1
                    if yStar < 28 and VisitedFlag[(xStar,yStar + 1)] == 0 and features[(xStar,yStar + 1)] == 0:
                        #print 4
                        L.enqueue(xStar)
                        L.enqueue(yStar + 1)
                        VisitedFlag[(xStar, yStar + 1)] = 1
...
def readCommand(argv):
    ...
    parser.add_option('-f', '--features', help=default('Whether to use enhanced features'), default=True,
                      action="store_true")
    ...
    parser.add_option('-a', '--autotune', help=default("Whether to automatically tune hyperparameters"), default=True,
                      action="store_true")
    parser.add_option('-i', '--iterations', help=default("Maximum iterations to run training"), default=3, type="int")
    parser.add_option('-s', '--test', help=default("Amount of test data to use"), default=TEST_SET_SIZE, type="int")
    parser.add_option('-%', '--percent', help=default("Percent of test data to use"), default="10", type=int)
    ...
    if (options.classifier == "naiveBayes" or options.classifier == "nb"):
        ...
        classifier = miras.MiraClassifier(legalLabels, options.iterations)
        if (options.autotune):
